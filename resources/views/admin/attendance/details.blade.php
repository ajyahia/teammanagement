...
@section('content')
    <div class="card" style="margin-bottom: 24px; background: linear-gradient(135deg, rgba(50, 138, 241, 0.05) 0%, rgba(33, 200, 246, 0.05) 100%);">
        <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 15px;">
            <div style="display: flex; align-items: center; gap: 15px;">
                ...
                <div>
                    <h2 style="font-family: var(--font-title); font-size: 1.35rem; margin-bottom: 4px;">{{ $employee->name }}</h2>
                    <p style="color: var(--text-secondary); font-size: 0.85rem;">
                        {{ $employee->job_title }} &bull; {{ __('Shift') }}: {{ $employee->work_start_time ? $employee->work_start_time->format('H:i') : '09:00' }} - {{ $employee->work_end_time ? $employee->work_end_time->format('H:i') : '17:00' }}
                    </p>
                </div>
            </div>
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
                ...
                <strong style="color: var(--color-primary-light); font-size: 1rem; text-transform: uppercase;">
                    {{ __(date('F', mktime(0, 0, 0, $month, 10, $year))) }} {{ $year }}
                </strong>
                <a href="/admin/attendance/months?month={{ $month }}&year={{ $year }}" class="btn btn-secondary" style="padding: 6px 12px; font-size: 0.8rem; margin-left: 10px;">
